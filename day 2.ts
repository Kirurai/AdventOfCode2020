...
let list = flag.map( el => el.split(" "));

let validPW = list.reduce( (acc, el) => {
    let min = parseInt(el[0].split("-")[0]);
    let max = parseInt(el[0].split("-")[1]);
    let letter = el[1][0];
    let word = el[2];
    let count = word.split("").filter( char => char === letter ).length;
